...
class CustomWeightNorm(WeightNorm):
    def __init__(self, name, dim):
        super(CustomWeightNorm,self).__init__(name,dim)
    ...
    def compute_weight(self, module):
        ...
        if(v.device.type != g.device.type):
            setattr(module, self.name  + '_g', torch.ones(v.shape[0], 1, 1, 1).cuda())
    ...
    @staticmethod
    def apply(module, name, dim,active_channels):
        ...
        setattr(module, name + '_g', torch.ones(weight.data.shape[0], 1, 1, 1))
...
def custom_weight_norm(module, active_channels,name='weight', dim=0):
    CustomWeightNorm.apply(module, name, dim,active_channels)
    return module
